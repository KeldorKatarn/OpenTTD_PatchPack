...
static const NWidgetPart _nested_departures_list[] = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_CAPTION, COLOUR_BROWN, WID_DB_CAPTION), SetDataTip(STR_DEPARTURES_CAPTION, STR_TOOLTIP_WINDOW_TITLE_DRAG_THIS),
	...
	NWidget(NWID_HORIZONTAL),
		NWidget(WWT_MATRIX, COLOUR_BROWN, WID_DB_LIST), SetMinimalSize(0, 0), SetFill(1, 0), SetResize(1, 1), SetScrollbar(WID_DB_SCROLLBAR),
		NWidget(NWID_VSCROLLBAR, COLOUR_BROWN, WID_DB_SCROLLBAR),
	EndContainer(),

	NWidget(NWID_HORIZONTAL),
		NWidget(WWT_PANEL, COLOUR_BROWN), SetMinimalSize(0, 12), SetResize(1, 0), SetFill(1, 1), EndContainer(),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_PAX), SetMinimalSize(6, 12), SetFill(0, 1), SetDataTip(STR_DEPARTURES_PAX, STR_DEPARTURES_PAX_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_FREIGHT), SetMinimalSize(6, 12), SetFill(0, 1), SetDataTip(STR_DEPARTURES_FREIGHT, STR_DEPARTURES_FREIGHT_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_ARRS), SetMinimalSize(6, 12), SetFill(0, 1), SetDataTip(STR_DEPARTURES_ARRIVALS, STR_DEPARTURES_ARRIVALS_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_DEPS), SetMinimalSize(6, 12), SetFill(0, 1), SetDataTip(STR_DEPARTURES_DEPARTURES, STR_DEPARTURES_DEPARTURES_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_VIA), SetMinimalSize(11, 12), SetFill(0, 1), SetDataTip(STR_DEPARTURES_VIA_BUTTON, STR_DEPARTURES_VIA_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_TRAINS), SetMinimalSize(14, 12), SetFill(0, 1), SetDataTip(STR_TRAIN, STR_STATION_VIEW_SCHEDULED_TRAINS_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_ROADVEHS), SetMinimalSize(14, 12), SetFill(0, 1), SetDataTip(STR_LORRY, STR_STATION_VIEW_SCHEDULED_ROAD_VEHICLES_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_SHIPS), SetMinimalSize(14, 12), SetFill(0, 1), SetDataTip(STR_SHIP, STR_STATION_VIEW_SCHEDULED_SHIPS_TOOLTIP),
		NWidget(WWT_TEXTBTN, COLOUR_BROWN, WID_DB_SHOW_PLANES),  SetMinimalSize(14, 12), SetFill(0, 1), SetDataTip(STR_PLANE, STR_STATION_VIEW_SCHEDULED_AIRCRAFT_TOOLTIP),
...
public:
		...
		switch (widget) {
			...
			case WID_DB_LIST: {  // Matrix to show departures
				...
				for (uint i = 0; i <= id_v; ++i) {
					...
					if (i == id_v) {
						const Vehicle* vehicle = Vehicle::GetIfValid(d->vehicle);						
	...
	virtual void OnTick()
	{
		...
		if (cached_date_width == 0 || _settings_client.gui.departure_show_both != cached_arr_dep_display_method) {
		...
		if (this->calc_tick_countdown <= 0) {
			...
			this->departures = (this->departure_types[0] ? MakeDepartureList(this->station, this->show_types, D_DEPARTURE, this->departure_types[2], show_pax, show_freight) : new DepartureList());
...
void DeparturesWindow::DrawDeparturesListItems(const Rect &r) const
{
	...
	for (uint i = 0; i < max_departures; ++i) {
		...
		if (arrival == this->arrivals->Length()) {
			assert(departure < this->departures->Length());
			d = (*(this->departures))[departure++];
		}
		else if (departure == this->departures->Length()) {
			assert(arrival < this->arrivals->Length());
			d = (*(this->arrivals))[arrival++];
		}
		else {
			assert(departure < this->departures->Length());
		...
		if ((d->scheduled_date / DEFAULT_DAY_TICKS) > MAX_DAY ||
			d->scheduled_date < 0) {
			continue;
		}

		if (d->terminus.station == INVALID_STATION) continue;
		...
		/* Time */
		const Order* departure_order = Order::GetIfValid(d->order);
		const Vehicle* departure_vehicle = Vehicle::GetIfValid(d->vehicle);

		if (departure_order == nullptr || departure_vehicle == nullptr) continue;

		SetDParam(0, d->scheduled_date);
		SetDParam(1, d->scheduled_date - departure_order->GetWaitTime());
		...
		if (_settings_client.gui.departure_show_vehicle_type) {
			...
			switch (departure_vehicle->type) {
			case VEH_TRAIN:
				type = STR_DEPARTURES_TYPE_TRAIN;
				break;
			case VEH_ROAD:
				type = IsCargoInClass(departure_vehicle->cargo_type, CC_PASSENGERS) ? STR_DEPARTURES_TYPE_BUS : STR_DEPARTURES_TYPE_LORRY;
				break;
			case VEH_SHIP:
		...
		const Station* terminus_station = Station::GetIfValid(d->terminus.station);

		if (_settings_client.gui.departure_destination_type && terminus_station != nullptr) {
			if (terminus_station->facilities & FACIL_DOCK &&
				terminus_station->facilities & FACIL_AIRPORT &&
				departure_vehicle->type != VEH_SHIP &&
				departure_vehicle->type != VEH_AIRCRAFT) {
				icon = STR_DEPARTURES_STATION_PORTAIRPORT;
			}
			else if (terminus_station->facilities & FACIL_DOCK &&
				departure_vehicle->type != VEH_SHIP) {
				icon = STR_DEPARTURES_STATION_PORT;
			}
			else if (terminus_station->facilities & FACIL_AIRPORT &&
				departure_vehicle->type != VEH_AIRCRAFT) {
				icon = STR_DEPARTURES_STATION_AIRPORT;
			}
		}

		const Station *via_station = Station::GetIfValid(d->via);

		if (_settings_client.gui.departure_destination_type && via_station != nullptr) {
			if (via_station->facilities & FACIL_DOCK &&
				via_station->facilities & FACIL_AIRPORT &&
				departure_vehicle->type != VEH_SHIP &&
				departure_vehicle->type != VEH_AIRCRAFT) {
				icon_via = STR_DEPARTURES_STATION_PORTAIRPORT;
			}
			else if (via_station->facilities & FACIL_DOCK &&
				departure_vehicle->type != VEH_SHIP) {
				icon_via = STR_DEPARTURES_STATION_PORT;
			}
			else if (via_station->facilities & FACIL_AIRPORT &&
				departure_vehicle->type != VEH_AIRCRAFT) {
		...
		/* Status */
		{
			...
			if (d->status == D_ARRIVED) {
				...
				if (d->lateness <= _settings_client.gui.ticks_per_minute && d->scheduled_date > GetCurrentTickCount()) {
					...
					if ((d->scheduled_date + d->lateness) < GetCurrentTickCount()) {
		...
		if (_settings_client.gui.departure_show_vehicle) {
			SetDParam(0, (uint64)(departure_vehicle->index));
		...
		if (_settings_client.gui.departure_show_group && departure_vehicle->group_id != INVALID_GROUP && departure_vehicle->group_id != DEFAULT_GROUP) {
			SetDParam(0, (uint64)(departure_vehicle->group_id));
		...
		if (_settings_client.gui.departure_show_company) {
			SetDParam(0, (uint64)(departure_vehicle->owner));
