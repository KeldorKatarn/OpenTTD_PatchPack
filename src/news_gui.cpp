/* $Id: news_gui.cpp 25668 2013-08-05 20:36:24Z michi_cc $ */
	...
	NWidget(WWT_PANEL, COLOUR_WHITE, WID_N_PANEL),
		NWidget(NWID_HORIZONTAL), SetPadding(1, 1, 0, 1),
			...
			NWidget(NWID_VERTICAL),
				NWidget(WWT_LABEL, COLOUR_WHITE, WID_N_DATE), SetDataTip(STR_TINY_BLACK_DATE_LONG, STR_NULL),
...
CommandCost CmdCustomNewsItem(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (company != INVALID_OWNER && !Company::IsValidID(company)) return CommandError();
	if (type >= NT_END) return CommandError();
	...
	switch (reftype1) {
		...
		case NR_VEHICLE:
			if (!Vehicle::IsValidID(p2)) return CommandError();
			break;

		case NR_STATION:
			if (!Station::IsValidID(p2)) return CommandError();
			break;
		...
		case NR_ENGINE:
			if (!Engine::IsValidID(p2)) return CommandError();
			break;
...
struct MessageHistoryWindow : Window {
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		if (widget == WID_MH_BACKGROUND) {
			...
			this->date_width = GetStringBoundingBox(STR_WHITE_DATE).width;
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		for (int n = this->vscroll->GetCapacity(); n > 0; n--) {
			SetDParam(0, ni->date);
			DrawString(date_left, date_right, y, STR_WHITE_DATE);
