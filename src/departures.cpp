...
typedef struct OrderDate
{
	...
	Ticks expected_date;    ///< The date on which the order is expected to complete
...
DepartureList* MakeDepartureList(StationID station, bool show_vehicle_types[5], DepartureType type, bool show_vehicles_via, bool show_pax, bool show_freight)
{
	...
	for (uint i = 0; i < 4; ++i) {
		...
		for (const Vehicle **v = vehicles.Begin(); v != vehicles.End(); v++) {
			...
			Ticks start_date = GetCurrentTickCount() - (*v)->current_order_time;
	...
	for(int i = 10000; i > 0; --i) {
		...
		d->scheduled_date = least_order->expected_date - least_order->lateness;
		d->lateness = least_order->lateness;
		d->status = least_order->status;
		d->vehicle = least_order->v->index;
		d->type = type;
		d->order = least_order->order->index;
		...
		for (uint i = 0; i < next_orders.Length(); ++i) {
			...
			Ticks lod = least_order->expected_date - least_order->lateness;
			Ticks odd = od->expected_date - od->lateness;
