	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		...
		case WID_PLN_RENAME: {
			if (_current_plan != nullptr) {
				SetDParamStr(0, _current_plan->GetNameCStr());
			...
			for (uint16 i = this->vscroll->GetPosition(); this->vscroll->IsVisible(i) && i < this->vscroll->GetCount(); i++) {
				...
				if (list[i].is_plan) {
					...
					if (!p->HasName()) {
						...
						SetDParamStr(0, p->GetNameCStr());
					}
					SetDParam(1, p->lines.size());
					SetDParam(2, p->creation_date);
					DrawString(text_left, text_right, y + (this->resize.step_height - FONT_HEIGHT_NORMAL) / 2, p->HasName() ? STR_PLANS_LIST_ITEM_NAMED_PLAN : STR_PLANS_LIST_ITEM_PLAN, p->visible_by_all ? TC_LIGHT_BLUE : TC_GOLD);
				}
				else {
					...
					DrawString(text_left, text_right, y + (this->resize.step_height - FONT_HEIGHT_NORMAL) / 2, STR_PLANS_LIST_ITEM_LINE, TC_BLACK);
				}
