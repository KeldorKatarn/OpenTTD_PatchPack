...
CommandCost CmdTurnRoadVeh(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (IsNormalRoadTile(v->tile) && GetDisallowedRoadDirections(v->tile) != DRD_NONE) return CommandError();

	if (IsTileType(v->tile, MP_TUNNELBRIDGE) && DirToDiagDir(v->direction) == GetTunnelBridgeDirection(v->tile)) return CommandError();
...
static Vehicle *EnumFindVehPreventingOvertake(Vehicle *v, void *data)
{
	...
	return is_preventing ? blocking_v : nullptr;
}
...
static Trackdir FollowPreviousRoadVehicle(const RoadVehicle *v, const RoadVehicle *prev, TileIndex tile, DiagDirection entry_dir, bool already_reversed)
{
	...
	if ((required & GetAnyRoadBits(tile, v->rtid.basetype, false)) == ROAD_NONE) {
...
again:
	...
	if (rd.x & RDE_TURNED) {
		...
		if (v->rtid.IsTram() && !IsRoadDepotTile(v->tile) && HasExactlyOneBit(GetAnyRoadBits(v->tile, ROADTYPE_TRAM, false))) {
			/*
