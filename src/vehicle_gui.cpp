 ...
 * @param show_template_replace If true include the template replace item.
...
struct RefitWindow : public Window {
	...
	RefitOption *GetRefitOption()
	{
		...
		if ((uint)this->sel[1] >= l.Length()) return nullptr;
				...
				switch (v->type) {
					case VEH_TRAIN: {
						...
						for (Train *u = Train::From(v); u != nullptr; u = u->Next()) {
							...
							/* Draw a selection. */
							if ((!vehicles_to_refit.Contains(u->index) || u->Next() == nullptr) && left != INT32_MIN) {
								if (u->Next() == nullptr && vehicles_to_refit.Contains(u->index)) {
	...
	for (uint i = this->vscroll->GetPosition(); i < max; ++i) {
		...
		switch (this->vehicles.SortType()) {		
			...
			case VST_CARGO: {
				...
				for (auto temp_v = v; temp_v != nullptr; temp_v = temp_v->Next()) {
			...
			case VST_VALUE: {
				Money total_value = 0;
				for (auto temp_v = v; temp_v != nullptr; temp_v = temp_v->GetNextVehicle()) {
...
static const NWidgetPart _nested_train_vehicle_details_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL),
		NWidget(WWT_MATRIX, COLOUR_GREY, WID_VD_MATRIX), SetResize(1, 1), SetMinimalSize(420, 45), SetMatrixDataTip(1, 0, STR_NULL), SetFill(1, 0), SetScrollbar(WID_VD_SCROLLBAR),
	...
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, WID_VD_DETAILS_TOTAL_CARGO), SetMinimalSize(99, 12),
		        SetDataTip(STR_VEHICLE_DETAIL_TAB_PERFORMANCE, STR_VEHICLE_DETAILS_TRAIN_PERFORMANCE_TOOLTIP), SetFill(1, 0), SetResize(1, 0),
		NWidget(WWT_RESIZEBOX, COLOUR_GREY),
	EndContainer(),
};
		...
		switch (widget) {
			case WID_VD_TOP_DETAILS: {
				...
				if (v->type == VEH_TRAIN) {
					...
					SetDParam(0, CeilDiv(gcache->cached_total_length * 10, TILE_SIZE));
					SetDParam(1, 1);
					SetDParam(2, v->GetDisplayProfitThisYear());
					SetDParam(3, v->GetDisplayProfitLastYear());
					SetDParam(4, v->GetDisplayProfitLifetime());
