...
CommandCost CmdBuildVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!IsDepotTile(tile) || !IsTileOwner(tile, _current_company)) return CommandError();

	VehicleType type = GetDepotVehicleType(tile);

	/* Validate the engine type. */
	EngineID eid = GB(p1, 0, 16);
	if (!IsEngineBuildable(eid, type, _current_company)) return CommandError(STR_ERROR_RAIL_VEHICLE_NOT_AVAILABLE + type);

	const Engine *e = Engine::Get(eid);
	CommandCost value(EXPENSES_NEW_VEHICLES, e->GetCost());

	/* Engines without valid cargo should not be available */
	if (e->GetDefaultCargoType() == CT_INVALID) return CommandError();
	...
	if (!Vehicle::CanAllocateItem(num_vehicles)) return CommandError(STR_ERROR_TOO_MANY_VEHICLES_IN_GAME);
	...
	if (unit_num == UINT16_MAX) return CommandError(STR_ERROR_TOO_MANY_VEHICLES_IN_GAME);
...
CommandCost CmdSellVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (front->vehstatus & VS_CRASHED) return CommandError(STR_ERROR_VEHICLE_IS_DESTROYED);

	/* Do this check only if the vehicle to be moved is non-virtual */
	if ( !HasBit(p1, 21) )
		if (!front->IsStoppedInDepot()) return CommandError(STR_ERROR_TRAIN_MUST_BE_STOPPED_INSIDE_DEPOT + front->type);
...
CommandCost CmdRefitVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!IsCompanyBuildableVehicleType(v->type)) return CommandError();
	...
	if (v != front && (v->type == VEH_SHIP || v->type == VEH_AIRCRAFT)) return CommandError();
	...
	if (!is_virtual_train) {
		if ((flags & DC_QUERY_COST) == 0 && // used by the refit GUI, including the order refit GUI.
			...
			return CommandError(STR_ERROR_TRAIN_MUST_BE_STOPPED_INSIDE_DEPOT + front->type);
		}
	...
	if (new_cid >= NUM_CARGO) return CommandError();
...
CommandCost CmdStartStopVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (v->vehstatus & VS_CRASHED) return CommandError(STR_ERROR_VEHICLE_IS_DESTROYED);

	switch (v->type) {
		case VEH_TRAIN:
			if ((v->vehstatus & VS_STOPPED) && Train::From(v)->gcache.cached_power == 0) return CommandError(STR_ERROR_TRAIN_START_NO_POWER);
			break;
		...
		case VEH_AIRCRAFT: {
			...
			if (!(v->vehstatus & VS_CRASHED) && a->state >= STARTTAKEOFF && a->state < TERM7) return CommandError(STR_ERROR_AIRCRAFT_IS_IN_FLIGHT);
			break;
		}
...
CommandCost CmdMassStartStopVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!vli.UnpackIfValid(p2)) return CommandError();
	if (!IsCompanyBuildableVehicleType(vli.vtype)) return CommandError();

	if (vehicle_list_window) {
		if (!GenerateVehicleSortList(&list, vli)) return CommandError();
	} else {
...
CommandCost CmdGroupSellAllVehicles(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!IsCompanyBuildableVehicleType(vehicle_type)) return CommandError();
...
{	
	...
	if (should_execute) {
		...
		if (!head) return CommandError(err);
...
CommandCost CmdCloneVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (v->type == VEH_TRAIN && (!v->IsFrontEngine() || Train::From(v)->crash_anim_pos >= 4400)) return CommandError();
 ...
 * @return 0 for success and CommandError() if no vehicle is able to go to depot
 */
static CommandCost SendAllVehiclesToDepot(DoCommandFlag flags, bool service, const VehicleListIdentifier &vli)
{
	...
	return had_success ? CommandCost() : CommandError();
}
...
CommandCost CmdSendVehicleToDepot(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	Vehicle *v = Vehicle::GetIfValid(GB(p1, 0, 20));
	if (v == nullptr) return CommandError();
	if (!v->IsPrimaryVehicle()) return CommandError();
...
CommandCost CmdRenameVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!reset) {
		if (Utf8StringLength(text) >= MAX_LENGTH_VEHICLE_NAME_CHARS) return CommandError();
		if (!(flags & DC_AUTOREPLACE) && !IsUniqueVehicleName(text)) return CommandError(STR_ERROR_NAME_MUST_BE_UNIQUE);
	}
...
CommandCost CmdChangeServiceInt(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (iscustom) {
		serv_int = GB(p2, 0, 16);
		if (serv_int != GetServiceIntervalClamped(serv_int, ispercent)) return CommandError();
	} else {
