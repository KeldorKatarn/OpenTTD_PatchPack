...
/** value drop down list for reserve through types strings and values */
...
static DropDownList *GetGroupDropDownList(Owner owner, GroupID group_id, int &selected)
{
	...
	const Group *g;
	FOR_ALL_GROUPS(g) {
		if (g->owner == owner && g->vehicle_type == VEH_TRAIN && g->statistics.num_vehicle != 0) {
			*list.Append() = g;
		}
	}
			...
			case TR_WIDGET_VALUE_DROPDOWN: {
				...
				switch (GetTraceRestrictTypeProperties(item).value_type) {
					...
					case TRVT_SLOT_INDEX: {
						...
						if (dlist != nullptr) ShowDropDownList(this, dlist, selected, TR_WIDGET_VALUE_DROPDOWN);
						break;
					}
			...
			case TR_WIDGET_LEFT_AUX_DROPDOWN: {
				...
				switch (GetTraceRestrictTypeProperties(item).value_type) {
					case TRVT_SLOT_INDEX_INT: {
						...
						if (dlist != nullptr) ShowDropDownList(this, dlist, selected, TR_WIDGET_LEFT_AUX_DROPDOWN);
						break;
					}
