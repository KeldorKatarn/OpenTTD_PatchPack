...
static void NPFFollowTrack(AyStar *aystar, OpenListNode *current)
{
	...
	bool ignore_src_tile = (current->path.parent == nullptr && NPFGetFlag(&current->path.node, NPF_FLAG_IGNORE_START_TILE));
	...
	while (trackdirbits != 0) {
		...
		if (IsTileType(dst_tile, MP_TUNNELBRIDGE) && IsTunnelBridgeSignalSimulationExit(dst_tile) && DiagDirToDiagTrackdir(GetTunnelBridgeDirection(dst_tile)) == dst_trackdir) {
			/* Entering a signaled bridge/tunnel from the wrong side, equivalent to encountering a one-way signal from the wrong side */
			break;
		}
		{
...
static NPFFoundTargetData NPFRouteToDepotBreadthFirstTwoWay(TileIndex tile1, Trackdir trackdir1, bool ignore_start_tile1, TileIndex tile2, Trackdir trackdir2, bool ignore_start_tile2, NPFFindStationOrTileData *target, AyStarUserData *user, uint reverse_penalty)
{
	...
	return NPFRouteInternal(&start1, ignore_start_tile1, (IsValidTile(tile2) ? &start2 : nullptr), ignore_start_tile2, target, NPFFindDepot, NPFCalcZero, user, reverse_penalty);
}
...
FindDepotData NPFRoadVehicleFindNearestDepot(const RoadVehicle *v, int max_penalty)
{
	...
	NPFFoundTargetData ftd = NPFRouteToDepotBreadthFirstTwoWay(v->tile, trackdir, false, v->tile, ReverseTrackdir(trackdir), false, nullptr, &user, 0);
