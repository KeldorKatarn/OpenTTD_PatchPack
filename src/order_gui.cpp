		...
		if (order == nullptr) {
			...
			switch (order->GetType()) {
				...
				case OT_CONDITIONAL: {
					...
					this->GetWidget<NWidgetCore>(WID_O_COND_COMPARATOR)->widget_data = GetComparatorStrings(order)[order == nullptr ? 0 : order->GetConditionComparator()];
		...
		switch (widget) {
			case WID_O_ORDER_LIST: {
				...
				if (_ctrl_pressed && _shift_pressed) {
					...
					if (selected_order != nullptr && selected_order->IsType(OrderType::OT_CONDITIONAL)) {
						VehicleOrderID order_id = this->GetOrderFromPt(_cursor.pos.y - this->top);
						if (order_id != INVALID_VEH_ORDER_ID) {
