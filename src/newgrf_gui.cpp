...
struct NewGRFParametersWindow : public Window {
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		if (widget == WID_NP_DESCRIPTION) {
			const GRFParameterInfo *par_info = (this->clicked_row < this->grf_config->param_info.Length()) ? this->grf_config->param_info[this->clicked_row] : nullptr;
		...
		for (uint i = this->vscroll->GetPosition(); this->vscroll->IsVisible(i) && i < this->vscroll->GetCount(); i++) {
			GRFParameterInfo *par_info = (i < this->grf_config->param_info.Length()) ? this->grf_config->param_info[i] : nullptr;
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_NP_BACKGROUND: {
				...
				GRFParameterInfo *par_info = (num < this->grf_config->param_info.Length()) ? this->grf_config->param_info[num] : nullptr;
	...
	virtual void OnQueryTextFinished(char *str)
	{
		...
		GRFParameterInfo *par_info = ((uint)this->clicked_row < this->grf_config->param_info.Length()) ? this->grf_config->param_info[this->clicked_row] : nullptr;
...
struct NewGRFWindow : public Window, NewGRFScanCallback {
	...
	void UpgradeCurrent()
	{
		...
		for (const GRFConfig *a = _all_grfs; a != nullptr; a = a->next) {
			...
			d->next = (*c)->next;
			d->CopyParams(**c);
			if (this->active_sel == *c) this->active_sel = nullptr;
			delete *c;
			*c = d;
			iter->second = d;
		}
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		...
		switch (widget) {
			...
			case WID_NS_PRESET_SAVE:
				ShowSavePresetWindow((this->preset == -1) ? nullptr : _grf_preset_list[this->preset]);
				break;
	...
	virtual void OnQueryTextFinished(char *str)
	{
		...
		for (uint i = 0; i < _grf_preset_list.Length(); i++) {
			if (_grf_preset_list[i] != nullptr && strcmp(_grf_preset_list[i], str) == 0) {
				this->preset = i;
				break;
			}
		}
...
private:
	...
	void BuildAvailables()
	{
		...
		if (this->avail_sel != nullptr) {
			...
			if (this->avail_pos < 0) this->avail_sel = nullptr;
		}
...
void ShowMissingContentWindow(const GRFConfig *list)
{
	...
	ShowNetworkContentListWindow(cv.Length() == 0 ? nullptr : &cv, CONTENT_TYPE_NEWGRF);
}
#endif
