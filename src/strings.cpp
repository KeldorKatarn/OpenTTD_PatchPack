...
#ifdef WITH_ICU_SORT
Collator *_current_collator = nullptr;               ///< Collator for the language currently in use.
#endif /* WITH_ICU_SORT */
		...
		switch (b) {
			...
			case SCC_VEHICLE_NAME: { // {VEHICLE}
				...
				if (v->name != nullptr) {
					...
					int64 args_array[] = {(uint64)(size_t)v->group_id, v->unitnumber};
					StringParameters tmp_params(args_array);
					buff = GetStringWithArgs(buff, STR_FORMAT_GROUP_VEHICLE, &tmp_params, last);
				} else {
