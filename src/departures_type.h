...
typedef struct CallAt {
	StationID station;
	Ticks scheduled_date;

	CallAt(const StationID& s) : station(s), scheduled_date(0) { }
	CallAt(const StationID& s, const Ticks& t) : station(s), scheduled_date(t) { }
...
typedef struct Departure {
	Ticks scheduled_date;              ///< The date this departure is scheduled to finish on (i.e. when the vehicle leaves the station)
	...
	VehicleID vehicle;                ///< The vehicle performing this departure
	OrderID order;                    ///< The order corresponding to this departure
	Departure() : terminus(INVALID_STATION), via(INVALID_STATION), calling_at(), vehicle(INVALID_VEHICLE), order(INVALID_ORDER) { }
	~Departure()
	{
		calling_at.Reset();
	}
    ...
    inline bool operator==(const Departure& d) const {
        ...
        return
            this->scheduled_date == d.scheduled_date &&
