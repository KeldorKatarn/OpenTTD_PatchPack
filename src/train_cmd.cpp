...
static CommandCost CmdBuildRailWagon(TileIndex tile, DoCommandFlag flags, const Engine *e, Vehicle **ret)
{
	...
	if (!IsCompatibleRail(rvi->railtype, GetRailType(tile))) return CommandError();
...
CommandCost CmdBuildRailVehicle(TileIndex tile, DoCommandFlag flags, const Engine *e, uint16 data, Vehicle **ret)
{
	...
	if (!HasPowerOnRail(rvi->railtype, GetRailType(tile))) return CommandError();
...
{
	for (; t != nullptr; t = t->Next()) *list.Append() = t;
}
...
static void InsertInConsist(Train *dst, Train *chain)
{
	...
	assert(dst->Next() == nullptr || !dst->Next()->IsArticulatedPart());
...
static CommandCost CheckTrainAttachment(Train *t)
{
	...
	while (t != nullptr) {
		...
		if (!t->IsArticulatedPart() && !t->IsRearDualheaded()) {
			...
			if (callback != CALLBACK_FAILED) {
				...
				if (error != STR_NULL) return CommandError(error);
			}
		}
	...
	if (allowed_len < 0) return CommandError(STR_ERROR_TRAIN_TOO_LONG);
	return CommandCost();
}
...
CommandCost CmdMoveRailVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (src == nullptr) return CommandError();

	CommandCost ret = CheckOwnership(src->owner);
	if (ret.Failed()) return ret;

	/* Do not allow moving crashed vehicles inside the depot, it is likely to cause asserts later */
	if (src->vehstatus & VS_CRASHED) return CommandError();
	...
	if (d == INVALID_VEHICLE) {
		...
		if (dst == nullptr) return CommandError();

		CommandCost ret = CheckOwnership(dst->owner);
		if (ret.Failed()) return ret;

		/* Do not allow appending to crashed vehicles, too */
		if (dst->vehstatus & VS_CRASHED) return CommandError();
	}
	...
	if (dst != nullptr) {
		...
		if (dst_head->tile != src_head->tile) return CommandError();
	...
	if (src->IsRearDualheaded()) return CommandError(STR_ERROR_REAR_ENGINE_FOLLOW_FRONT);
	...
	if ( !HasBit(p1, 21) )
		if (!src_head->IsStoppedInDepot()) return CommandError(STR_ERROR_TRAINS_CAN_ONLY_BE_ALTERED_INSIDE_A_DEPOT);

	/* Check if all vehicles in the destination train are stopped inside a depot. */
	/* Do this check only if the destination vehicle is non-virtual */
	if ( !HasBit(p1, 21) )
		if (dst_head != nullptr && !dst_head->IsStoppedInDepot()) return CommandError(STR_ERROR_TRAINS_CAN_ONLY_BE_ALTERED_INSIDE_A_DEPOT);
...
CommandCost CmdSellRailWagon(DoCommandFlag flags, Vehicle *t, uint16 data, uint32 user)
{
	...
	if (v->IsRearDualheaded()) return CommandError(STR_ERROR_REAR_ENGINE_FOLLOW_FRONT);
...
CommandCost CmdReverseTrainDirection(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (p2 != 0) {
		...
		if (v->IsMultiheaded() || HasBit(EngInfo(v->engine_type)->callback_mask, CBM_VEHICLE_ARTIC_ENGINE)) {
			return CommandError(STR_ERROR_CAN_T_REVERSE_DIRECTION_RAIL_VEHICLE_MULTIPLE_UNITS);
		}
		if (!HasBit(EngInfo(v->engine_type)->misc_flags, EF_RAIL_FLIPS)) return CommandError();

		Train *front = v->First();
		/* make sure the vehicle is stopped in the depot */
		if (!front->IsStoppedInDepot()) {
			return CommandError(STR_ERROR_TRAINS_CAN_ONLY_BE_ALTERED_INSIDE_A_DEPOT);
		}
		...
		if ((v->vehstatus & VS_CRASHED) || v->breakdown_ctr != 0) return CommandError();
...
CommandCost CmdForceTrainProceed(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!t->IsPrimaryVehicle()) return CommandError();
 ...
 * @param location [out]    If not \c nullptr and a depot is found, store its location in the given address.
 * @param destination [out] If not \c nullptr and a depot is found, store its index in the given address.
 * @param reverse [out]     If not \c nullptr and a depot is found, store reversal information in the given address.
		...
		if (IsTileType(tile, MP_RAILWAY)) {
			...
			if (HasPbsSignalOnTrackdir(tile, td)) {
				if (GetSignalStateByTrackdir(tile, td) == SIGNAL_STATE_RED) {
					...
					SignalStateChanged(tile, TrackdirToTrack(td), 1);
				}
...
static Track ChooseTrainTrack(Train *v, TileIndex tile, DiagDirection enterdir, TrackBits tracks, bool force_res, bool *p_got_reservation, bool mark_stuck)
{
	...
	if (KillFirstBit(tracks) == TRACK_BIT_NONE) {
		...
		if (track != INVALID_TRACK && HasPbsSignalOnTrackdir(tile, TrackEnterdirToTrackdir(track, enterdir))) {
			...
			/* notify logic signals of the state change */
			SignalStateChanged(tile, TrackdirToTrack(TrackEnterdirToTrackdir(track, enterdir)), 1);
	...
	if (do_track_reservation) {
		...
		if (res_dest.tile == INVALID_TILE) {
			...
			if (changed_signal) {
				...
				/* notify logic signals of the state change */
				SignalStateChanged(tile, TrackdirToTrack(TrackEnterdirToTrackdir(best_track, enterdir)), 1);
			}
...
static Vehicle *CheckTrainAtSignal(Vehicle *v, void *data)
{
	...
	if (t->cur_speed > 5 || TrainExitDir(t->direction, t->track) != exitdir) return nullptr;
	...
	for (prev = v->Previous(); v != nomove; prev = v, v = v->Next()) {
		...
		if (v->track != TRACK_BIT_WORMHOLE) {
			...
			if (gp.old_tile == gp.new_tile) {
				...
				if (_settings_game.pf.forbid_90_deg && prev == nullptr) {
				...
				if (prev == nullptr) {
					...
					Track chosen_track_num = ChooseTrainTrack(v, gp.new_tile, enterdir, bits, false, nullptr, true);
				...
				if (!HasBit(r, VETS_ENTERED_WORMHOLE)) {
					...
					if (v->IsFrontEngine() && HasPbsSignalOnTrackdir(gp.new_tile, tdir)) {
						...
						/* notify logic signals of the state change */
						SignalStateChanged(gp.new_tile, TrackdirToTrack(tdir), 1);
					}

					/* Clear any track reservation when the last vehicle leaves the tile */
					if (v->Next() == nullptr) ClearPathReservation(v, v->tile, v->GetVehicleTrackdir(), true);
			...
			if (old_tile != gp.new_tile && IsTunnelBridgeWithSignalSimulation(v->tile) && (v->IsFrontEngine() || v->Next() == nullptr)) {
				if (old_tile == v->tile) {
					if (v->IsFrontEngine() && v->force_proceed == 0 && IsTunnelBridgeSignalSimulationExit(v->tile))
						goto invalid_rail;
...
CommandCost CmdBuildVirtualRailVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (!IsEngineBuildable(eid, VEH_TRAIN, _current_company)) {
		return CommandError(STR_ERROR_RAIL_VEHICLE_NOT_AVAILABLE + VEH_TRAIN);
	}
	...
	if (!need_replacement) {
		...
		if (!buyCost.Succeeded() || !CheckCompanyHasMoney(buyCost)) {
			...
			if (!buyCost.Succeeded() && buyCost.GetErrorMessage() != INVALID_STRING_ID) {
				...
				return CommandError(STR_ERROR_NOT_ENOUGH_CASH_REQUIRES_CURRENCY);
			}
		}
	}
