	...
	has_param_defaults(config.has_param_defaults)
{
	...
	for (uint i = 0; i < config.param_info.Length(); i++) {
		if (config.param_info[i] == nullptr) {
			*this->param_info.Append() = nullptr;
		} else {
...
void DoScanNewGRFFiles(void *callback)
{
	...
	if (callback != nullptr) ((NewGRFScanCallback*)callback)->OnNewGRFsScanned();
...
void ScanNewGRFFiles(NewGRFScanCallback *callback)
{
	...
	if (!VideoDriver::GetInstance()->HasGUI() || !ThreadObject::New(&DoScanNewGRFFiles, callback, nullptr, "ottd:newgrf-scan")) {
		_modal_progress_work_mutex->EndCritical();
		_modal_progress_paint_mutex->EndCritical();
