...
{
	return this->GetRunningCost() * _settings_game.economy.daylength;
}
...
CommandCost CmdSetVehicleVisibility(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (e == nullptr || _current_company >= MAX_COMPANIES) return CommandError();
	if (!IsEngineBuildable(e->index, e->type, _current_company)) return CommandError();
...
CommandCost CmdWantEnginePreview(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (e == nullptr || !(e->flags & ENGINE_EXCLUSIVE_PREVIEW) || e->preview_company != _current_company) return CommandError();
...
{
	Engine *e = Engine::GetIfValid(p1);
	if (e == nullptr) return CommandError();

	bool reset = StrEmpty(text);

	if (!reset) {
		if (Utf8StringLength(text) >= MAX_LENGTH_ENGINE_NAME_CHARS) return CommandError();
		if (!IsUniqueEngineName(text)) return CommandError(STR_ERROR_NAME_MUST_BE_UNIQUE);
	}
