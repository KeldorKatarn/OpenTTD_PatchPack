...
static Window *_mouseover_last_w = nullptr; ///< Window of the last #MOUSEOVER event.
 ...
 */
static SmallVector<WindowDesc*, 16> *_window_descs = nullptr;
...
{
	if (_window_descs == nullptr) _window_descs = new SmallVector<WindowDesc*, 16>();
...
void WindowDesc::LoadFromConfig()
{
	...
	for (WindowDesc **it = _window_descs->Begin(); it != _window_descs->End(); ++it) {
		if ((*it)->ini_key == nullptr) continue;
...
static void DispatchToolTipEvent(Window *w, int x, int y, TooltipCloseCondition close_cond)
{
	...
	/* No widget to handle, or the window is not interested in it. */
	if (wid != nullptr && wid->index >= 0) {
		Point pt = { x, y };
		w->OnToolTip(pt, wid == nullptr ? -1 : wid->index, close_cond);
	}
}
...
static Point LocalGetWindowPlacement(const WindowDesc *desc, int16 sm_width, int16 sm_height, int window_number)
{
	...
	if (desc->parent_cls != 0 /* WC_MAIN_WINDOW */ && (w = FindWindowById(desc->parent_cls, window_number)) != nullptr) {
		bool rtl = _current_text_dir == TD_RTL;
...
static EventState HandleViewportScroll()
{
	...
	if (_last_scroll_window == nullptr || !(_right_button_down || scrollwheel_scrolling || (_settings_client.gui.left_mouse_btn_scrolling && _left_button_down))) {
	...
	if (delta.x != 0 || delta.y != 0) {
		...
		if (vp == nullptr || scrollwheel_scrolling ||
	...
	if (vp != nullptr) {
		...
		switch (click) {
			...
			case MC_HOVER:
				...
				if (_cursor.delta.x == 0 && _cursor.delta.y == 0 && FindWindowById(WC_TOOLTIPS, 0) != nullptr) break;
				HandleViewportToolTip(w, x, y);
				break;
