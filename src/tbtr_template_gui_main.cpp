...
static const NWidgetPart _widgets[] = {
	...
	NWidget(NWID_VERTICAL),
		...
		NWidget(NWID_HORIZONTAL),
			...
			/*NWidget(WWT_PUSHTXTBTN, COLOUR_GREY, TRW_WIDGET_TMPL_BUTTONS_RPLALL), SetMinimalSize(150,12), SetResize(0,0), SetDataTip(STR_TMPL_RPL_ALL_TMPL, STR_TMPL_RPL_ALL_TMPL),*/
...
public:
	TemplateReplaceWindow(WindowDesc *wdesc, byte dig, int step_h) : Window(wdesc)
	{
		...
		//this->SetWidgetDisabledState(TRW_WIDGET_TMPL_BUTTONS_RPLALL, _networking);
	...
	void DrawTemplateInfo(int line_height, const Rect &r) const
	{
		...
		DrawString(8, r.right, ScaleGUITrad(4) - this->vscroll[2]->GetPosition(), STR_VEHICLE_INFO_LOADED_WEIGHT_POWER_MAX_SPEED_MAX_TE);
