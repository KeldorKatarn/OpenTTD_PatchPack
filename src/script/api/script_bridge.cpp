...
/* static */ bool ScriptBridge::_BuildBridgeRoad1()
{
	...
	return ScriptObject::DoCommand(start + ::TileOffsByDiagDir(dir_1), ::DiagDirToRoadBits(dir_2) | (ScriptObject::GetRoadType().Pack() << 4) | (1 << 12), 0, CMD_BUILD_ROAD, NULL, &::_DoCommandReturnBuildBridge2);
}
...
/* static */ bool ScriptBridge::_BuildBridgeRoad2()
{
	...
	return ScriptObject::DoCommand(end + ::TileOffsByDiagDir(dir_2), ::DiagDirToRoadBits(dir_1) | (ScriptObject::GetRoadType().Pack() << 4) | (1 << 12), 0, CMD_BUILD_ROAD);
}
