/* $Id: newgrf.cpp 26245 2014-01-12 18:01:50Z frosch $ */
...
static ChangeInfoResult AirportChangeInfo(uint airport, int numinfo, int prop, ByteReader *buf)
{
	...
	for (int i = 0; i < numinfo; i++) {
		...
		switch (prop) {
			...
			case 0x12: { // Accept seaplanes
				byte is_seaplane = buf->ReadByte();
				if (is_seaplane == 0x01){
					as->cls_id = APC_SEA;
				}
				break;
...
static void NewSpriteGroup(ByteReader *buf)
{
	...
	switch (type) {
		...
		case 0x8A: // Parent scope, dword
		{
			...
			group->num_ranges = (uint)optimised.size();
	...
	FOR_ALL_ENGINES(e) {
		...
		if (ei->cargo_type == CT_INVALID && ei->refit_mask != 0) {
			...
			const uint8 *cargo_map_for_first_refittable = nullptr;
			{
				...
				if (file != nullptr && file->grf_version >= 8 && file->cargo_list.Length() != 0) {
