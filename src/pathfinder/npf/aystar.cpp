...
{
	auto result = this->closedlist_hash.find(std::make_pair(node->tile, node->direction));
...
{
	auto result = this->openlist_hash.find(std::make_pair(node->tile, node->direction));
