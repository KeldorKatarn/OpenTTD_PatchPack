...
{
	if (this->orders.list != nullptr) this->orders.list->MarkSeparationInvalid();
}

/** Sets new separation settings for this vehicle's shared orders. */
void Vehicle::SetSepSettings(TTSepMode Mode, uint Parameter)
{
	if (this->orders.list != nullptr) this->orders.list->SetSepSettings(Mode, Parameter);
}
...
CommandCost EnsureNoVehicleOnGround(TileIndex tile)
{
	...
	if (v != nullptr) return CommandError(STR_ERROR_TRAIN_IN_THE_WAY + v->type);
	return CommandCost();
}
...
void VehicleEnterDepot(Vehicle *v)
{
	...
	if (v->current_order.IsType(OT_GOTO_DEPOT)) {
		...
		/* Handle the ODTFB_PART_OF_ORDERS case. If there is a timetabled wait time, hold the vehicle, otherwise skip to the next order.
		Note that if there is a only a travel_time, but no wait_time defined for the order, and the vehicle arrives to the depot sooner as scheduled,
		he doesn't wait in it, as it would in stations. Thus, the original behavior is maintained if there's no defined wait_time.*/
		if (v->current_order.GetDepotOrderType() & ODTFB_PART_OF_ORDERS) {
...
{
	::MarkAllViewportsDirty(this->coord.left, this->coord.top, this->coord.right, this->coord.bottom, this->type != VEH_EFFECT ? ZOOM_LVL_END : ZOOM_LVL_DRAW_MAP);
}
...
void Vehicle::LeaveStation()
{
	...
	for (int i = this->GetNumOrders() - 1; i >= 0; --i) {
		...
		if ((order->IsType(OT_GOTO_STATION) || order->IsType(OT_IMPLICIT)) &&
			...
			if (order->GetUnloadType() == OUFB_CARGO_TYPE_UNLOAD || order->GetLoadType() == OLFB_CARGO_TYPE_LOAD) {
				can_load_or_unload = true;
			} else if ((order->GetLoadType() & OLFB_NO_LOAD) == 0 || (order->GetUnloadType() & OUFB_NO_UNLOAD) == 0) {
				can_load_or_unload = true;
			}
		}

		if (can_load_or_unload && !(order->GetLoadType() & OLFB_NO_LOAD)) {
	...
	if (last_loading_order_index >= 0 && last_loading_order_index < this->GetNumOrders()) {
		...
		if ((current_real_order->IsType(OT_GOTO_STATION) || current_real_order->IsType(OT_IMPLICIT)) &&
			(current_real_order->GetNonStopType() & ONSF_NO_STOP_AT_DESTINATION_STATION) == 0) {
			if (current_real_order->GetUnloadType() == OUFB_CARGO_TYPE_UNLOAD || current_real_order->GetLoadType() == OLFB_CARGO_TYPE_LOAD) {
				can_load_or_unload = true;
			}
			else if ((current_real_order->GetLoadType() & OLFB_NO_LOAD) == 0 || (current_real_order->GetUnloadType() & OUFB_NO_UNLOAD) == 0) {
				can_load_or_unload = true;
			}
		}

		bool current_order_was_load_order = (can_load_or_unload && !(current_real_order->GetLoadType() & OLFB_NO_LOAD));

		if (current_order_was_load_order)
		{
			int8 occupancy = (int8)CalcPercentVehicleFilled(this, nullptr);
...
CommandCost Vehicle::SendToDepot(DoCommandFlag flags, DepotCommand command)
{
	...
	if (this->vehstatus & VS_CRASHED) return CommandError();
	if (this->IsStoppedInDepot()) return CommandError();
	...
	if (HasBit(this->vehicle_flags, VF_SHOULD_GOTO_DEPOT) || HasBit(this->vehicle_flags, VF_SHOULD_SERVICE_AT_DEPOT)) {
		...
		if (command & DEPOT_DONT_CANCEL) return CommandError(); // Requested no cancellation of depot orders
	...
	for (int i = 0; i < this->GetNumOrders(); ++i) {
		...
		if (isRegularOrder && isDepotOrder) {
			...
			if (this->type == VEH_AIRCRAFT) {
				...
				if (st != nullptr && st->airport.HasHangar() && CanVehicleUseStation(this, st)) {
					location = st->xy;
	...
	if (!foundDepotInOrders && !this->FindClosestDepot(&location, &destination, &reverse)) return CommandError(no_depot[this->type]);
