...
static void IniLoadSettingList(IniFile *ini, const char *grpname, StringList *list)
{
	...
	if (group == nullptr || list == nullptr) return;

	list->Clear();

	for (const IniItem *item = group->item; item != nullptr; item = item->next) {
		if (item->name != nullptr) *list->Append() = stredup(item->name);
	}
}
...
void GetGRFPresetList(GRFPresetList *list)
{
	...
	IniFile *ini = IniLoadConfig();
	IniGroup *group;
	for (group = ini->group; group != nullptr; group = group->next) {
...
CommandCost CmdChangeSetting(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (sd == nullptr) return CommandError();
	if (!SlIsObjectCurrentlyValid(sd->save.version_from, sd->save.version_to)) return CommandError();

	if (!sd->IsEditable(true)) return CommandError();
...
{
	if (p1 >= lengthof(_company_settings)) return CommandError();
