	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		...
		switch (widget) {
			...
			case WID_ROT_BUILD_TUNNEL:
				HandlePlacePushButton(this, WID_ROT_BUILD_TUNNEL, GetRoadTypeInfo(roadtype_identifier)->cursor.tunnel, HT_SPECIAL | HT_TUNNEL);
				this->last_started_action = widget;
				break;
		...
		if (pt.x != -1) {
			switch (select_proc) {
				...
				case DDSP_PLACE_AUTOROAD:
					...
					DoCommandP(start_tile, end_tile, _place_road_flag | (_cur_roadtype_identifier.Pack() << 3) | (_one_way_button_clicked << 9),
