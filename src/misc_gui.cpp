...
static const char * const _credits[] = {
	...
	"  Andreas Schmitt (Lt_Joker)",
	"",
	"",
	"Several features imported from the JGR Patch Pack developed by:",
	...
	virtual void OnInit() {
		...
		if (HasBit(cs->callback_mask, CBM_CARGO_STATION_RATING_CALC)) {
			...
			if (callback != CALLBACK_FAILED) {
				...
				SetDParam(0, min(ge->last_unprocessed_speed, 0xFF));
				...
				GetString(this->data[line_nr],
						  (ge->punishment_in_effect || ge->punishment_triggered) ?
					          STR_STATION_RATING_TOOLTIP_NEWGRF_WAITUNITS_FEEDBACK :
		...
		if (!this->newgrf_rating_used) {
			...
			GetString(this->data[line_nr],
				      (ge->punishment_in_effect || ge->punishment_triggered) ? STR_STATION_RATING_TOOLTIP_WAITUNITS_FEEDBACK : STR_STATION_RATING_TOOLTIP_WAITUNITS,
					  lastof(this->data[line_nr]));
			line_nr++;
			...
			if (ge->last_speed == 255) {
				...
				SetDParam(0, STR_STATION_RATING_TOOLTIP_SPEED_0 + min(3, r_speed / 15));
			}
			SetDParam(1, ge->last_unprocessed_speed);
