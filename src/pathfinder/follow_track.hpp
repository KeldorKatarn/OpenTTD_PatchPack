...
struct CFollowTrackT
{
	...
	{
		assert((!IsRoadTT() || m_veh != nullptr) && (!IsRailTT() || railtype_override != INVALID_RAILTYPES));
		m_veh_owner = o;
		m_pPerf = pPerf;
	...
	inline bool Follow(TileIndex old_tile, Trackdir old_td)
	{
		...
		m_old_td = old_td;
		m_err = EC_NONE;
		assert((IsTram() && GetSingleTramBit(m_old_tile) != INVALID_DIAGDIR) || // Disable the assertion for single tram bits
			   ((TrackStatusToTrackdirBits(GetTileTrackStatus(m_old_tile, TT(), IsRoadTT() ? RoadVehicle::From(m_veh)->compatible_roadtypes : 0)) & TrackdirToTrackdirBits(m_old_td)) != 0));
